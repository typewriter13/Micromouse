...
esp_err_t get_handler(httpd_req_t *req)
{
    const char resp[] =
        ...
        "        <hr>\n"
        "        <b>Wall Stop Distance (mm):</b><br>\n"
        "        Wall Distance: <input type=\"number\" step=\"1\" id=\"wdInput\" value=\"42\"><br><br>\n"
        "        <hr>\n"
        "        <b>Duty Cycle Limits:</b><br>\n"
        "        Max Duty Cycle: <input type=\"number\" step=\"0.1\" id=\"maxInput\" value=\"70\"><br><br>\n"
        "        Min Duty Cycle: <input type=\"number\" step=\"0.1\" id=\"minInput\" value=\"0\"><br><br>\n"
        ...
        "            var wdVal = document.getElementById(\"wdInput\").value;\n"
        "            var maxVal = document.getElementById(\"maxInput\").value;\n"
        "            var minVal = document.getElementById(\"minInput\").value;\n"
        "\n"
        "            var params = \"kp=\" + kpVal + \"&kd=\" + kdVal + \"&lt=\" + ltVal + \"&rt=\" + rtVal\n"
        "                       + \"&wd=\" + wdVal + \"&max=\" + maxVal + \"&min=\" + minVal;\n"
        "\n"
...
esp_err_t get_handler_str(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "wd", param, sizeof(param)) == ESP_OK) {
                wall_distance = atoi(param);
                ESP_LOGI(TAG_WIFI, "New Wall Distance: %d", wall_distance);
            }
            if (httpd_query_key_value(buf, "max", param, sizeof(param)) == ESP_OK) {
                max_duty_cycle = atof(param);
                ESP_LOGI(TAG_WIFI, "New Max Duty Cycle: %f", max_duty_cycle);
            }
            if (httpd_query_key_value(buf, "min", param, sizeof(param)) == ESP_OK) {
                min_duty_cycle = atof(param);
                ESP_LOGI(TAG_WIFI, "New Min Duty Cycle: %f", min_duty_cycle);
            }
        }
        free(buf);
    }
    ...
    snprintf(resp, sizeof(resp),
        "Updated! Kp: %.4f, Kd: %.4f, L_Thresh: %d, R_Thresh: %d, Wall: %d, Max: %.1f, Min: %.1f",
        Kp, Kd, left_turn_threshold, right_turn_threshold, wall_distance, max_duty_cycle, min_duty_cycle);
    httpd_resp_send(req, resp, HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
