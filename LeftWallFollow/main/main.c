...
volatile float max_duty_cycle = 60.0f;
volatile float min_duty_cycle = 10.0f;
volatile int wall_distance = 42;
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        if (timer <= 0)
        {
            ...
            if (left_confirmed)
            {
                current_state = bot_turn_left;
            }
            else if (front_open)
            {
                current_state = bot_pd;
            }
            else if (right_confirmed)
            {
                current_state = bot_turn_right;
            }
            else if (front_close && !right_confirmed && !left_confirmed)
            {
                current_state = bot_turn_around;
            }
            else
            {
                current_state = bot_brake;
            }
