...
// How often the main loop re-checks sensors
...
// Both motors mounted facing the SAME way (not mirrored) — so both timers
// are driven via the same OPR channel (OPR_A) to spin the same real-world
// direction, instead of the mirrored OPR_A/OPR_B split used before.
...
void pwm_motors_stop(void)
{
    ...
    if (motors_running) {
        ESP_LOGI(TAG, "Both motors stopped.");
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        // ToF is still read and logged, but no longer controls the motors
        ...
        ESP_LOGI(TAG, "IR S1: %d  IR S2: %d", adc_value_1, adc_value_2);
        printf("-----------------------------------------------------------------\n");
