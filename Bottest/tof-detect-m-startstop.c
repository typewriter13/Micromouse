...
// Both motors driven together (mirror-mounted, so opposite MCPWM channels
// move the bot straight). Duty registers written back-to-back so both
// motors start on the same cycle.
...
void pwm_motors_stop(void)
{
    ...
    if (motors_running) {
        ESP_LOGI(TAG, "Both motors stopped — wall detected within %d mm.", WALL_STOP_DISTANCE_MM);
