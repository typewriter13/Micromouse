...
#define WALL_STOP_DISTANCE_MM  100
...
// Front pair drives PID centering.
#define ADC_PIN_FRONT_LEFT     ADC_CHANNEL_9   // GPIO10 — front LEFT
#define ADC_PIN_FRONT_RIGHT    ADC_CHANNEL_8   // GPIO9  — front RIGHT
#define ADC_PIN_DIAG_RIGHT     ADC_CHANNEL_7   // GPIO8  — right diagonal
#define ADC_PIN_DIAG_LEFT      ADC_CHANNEL_2   // GPIO3  — left diagonal
#define ADC_UNIT      ADC_UNIT_1
#define ADC_BITWIDTH  ADC_BITWIDTH_12
#define ADC_ATTEN     ADC_ATTEN_DB_12

//PID gains for wall centering
#define PID_KP   0.08f
#define PID_KI   0.0f
#define PID_KD   0.05f
...
//PID wall centering (front sensor pair only)
...
void initialize_adc()
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, ADC_PIN_FRONT_LEFT, &config));
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, ADC_PIN_FRONT_RIGHT, &config));
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, ADC_PIN_DIAG_RIGHT, &config));
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, ADC_PIN_DIAG_LEFT, &config));
}
...
void app_main(void)
{
    ...
    uint8_t dist_data[2];
    const float dt_seconds = 0.1f;

    while (1)
    {
        int ir_front_left  = read_adc(ADC_PIN_FRONT_LEFT);
        int ir_front_right = read_adc(ADC_PIN_FRONT_RIGHT);
        int ir_diag_right  = read_adc(ADC_PIN_DIAG_RIGHT);
        int ir_diag_left   = read_adc(ADC_PIN_DIAG_LEFT);
        ...
        if (tof_distance_mm > LOWER_THRESHOLD && tof_distance_mm < UPPER_THRESHOLD) {
            if (tof_distance_mm <= WALL_STOP_DISTANCE_MM) {
                ...
                float correction = pid_compute_correction(ir_front_left, ir_front_right, dt_seconds);
        ...
        ESP_LOGI(TAG, "IR Front L: %d  Front R: %d  Diag L: %d  Diag R: %d",
                 ir_front_left, ir_front_right, ir_diag_left, ir_diag_right);
        printf("-----------------------------------------------------------------\n");
